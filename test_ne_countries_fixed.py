    ...
    for feature in country_geojson['features']:
        ...
        for field in fields_to_check:
            ...
            if isinstance(val, str) and target.lower() in val.lower():
                print(f"[OK] Found '{target}' in property '{field}': {val}")
                found = True
                break
        if found:
            break
    if not found:
        print(f"[NOT FOUND] '{target}' NOT FOUND in Natural Earth 110m")
    ...
    for target in targets:
        ...
        for feature in country_geojson_50m['features']:
            ...
            for field in fields_to_check:
                val = props.get(field, '')
                if isinstance(val, str) and target.lower() in val.lower():
                    print(f"[OK] Found '{target}' in 50m property '{field}': {val}")
                    found = True
                    break
            if found:
                break
        if not found:
            print(f"[NOT FOUND] '{target}' NOT FOUND in 50m either")
